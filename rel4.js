...
function validate_register_add_property_form(that) {
    ...
    var req_img = 'wp-rem-gallery-holder',
            _this_form = $(that),
    ...
    _this_form.find('.' + req_img).wp_rem_req_loop(function (element, index, set) {

        if($(element).data('allow-ext') !== 'txt,pdf,doc,docx')
        {
            jQuery(".gal-img").find("img").each(function(){
                var src = $(this).attr('src');
                var fext = src.split(';').shift();
                fext = fext.split('/').pop();
                if(fileExtension.indexOf(fext) > -1){
            ...
            if (img_count == 0) {
                ...
                $("html,body").animate({scrollTop: $(".property-settings-nav").offset().top }, "slow");
            }
        }
...
function property_ajax_pkg_activation_msg(that) {
    ...
    $.ajax({
        ...
        if ($('li .check-box-remind input').length > 0) {
            $('.check-box-remind').closest('li').hide();
        }
        var response = {
            type: 'success',
...
function add_event_listners(strings, $) {
    ...
    $("#btn-next-advance-options").click(function (e) {
        var this_id = $(this).data('id');
        var loc_on_off  =   $('.wp-rem-dev-select-type:checked').attr('data-loc');
        var is_form_valid = validate_register_add_property_form($(this).parents('ul#wp-rem-dev-main-con-' + this_id));

        if (is_form_valid) {
            if(loc_on_off == 'off'){
                change_tab('property-photos', e);
                $('li[data-act="loc-address"]').addClass('active');
            }else{
    ...
    $("#btn-back-property-photos").click(function (e) {
        ...
        if(loc_on_off == 'off'){
            change_tab('advance-options', e);
            $('li[data-act="loc-address"]').removeClass('active');
        }else{
            change_tab('loc-address', e);
        }
    ...
    $(document).on('click', '#register-property-order', function (e) {
        ...
        if ($("input[name='wp_rem_property_package']:checked").length > 0 && $("input[name='wp_rem_property_package']:checked").parents('td').find('a').attr('data-ppric') == 'free') {
            ...
            free_pkg = 'true';
        } else if ($("input[name='wp_rem_property_package']:checked").length === 0 && $("input[name='wp_rem_property_active_package']:checked").length === 0) {
            free_pkg = 'true';
        }
    ...
    function process_form_add_property_and_register_user(strings, package_verification, do_update) {
        ...
        if ($('.wp_rem_property_ploor_plan_attachment').length) {
            var floor_files = $('.wp_rem_property_ploor_plan_attachment').prop('files');
            if (floor_files.length > 0) {
                $.each($('.wp_rem_property_ploor_plan_attachment'), function (i, obj) {
                    $.each(obj.files, function (j, file) {
                        data.append('wp_rem_property_ploor_plan_attachment[' + j + ']', file);
                    })
                });
            }
        }
                    ...
                    if (response.status == true) {
                        if (typeof response.payment_gateway != "undefined") {
                            if (response.payment_gateway == "WP_REM_PRE_BANK_TRANSFER") {
                                $(".wp-rem-dev-property-form").html(response.msg);
                            } else {
                                $(".wp-rem-dev-property-form").hide();
...
$(document).on('click', '.wp-rem-dev-select-type', function () {
    ...
    var selecting_type,
            ...
            tags_append = $('#wp-rem-proprty-tags-holder'),
            after_append = $('.wp-rem-dev-appended-cats'),
            price_append = $('.wp-rem-dev-appended-price'),
            features_append = $('.wp-rem-append-features-check-list'),
            pckgs_append = $('#property-membership-info-main'),
            loc_append  =   $('.loc-address-tab-container'),
    ...
    selecting_type = $.ajax({
        ...
        loc_append.find('.wp-rem-dev-appended').remove();
        if (typeof response.loc_html !== 'undefined') {
            loc_append.prepend(response.loc_html);
        }
        ...
        if (response.detail_options.gallery == 'on') {
            jQuery('#wp-rem-property-gallery-holder').show();
        } else {
            jQuery('#wp-rem-property-gallery-holder').hide();
        }
        if (response.detail_options.attachments == 'on') {
            jQuery('#wp-rem-property-attachments-holder').show();
        } else {
            jQuery('#wp-rem-property-attachments-holder').hide();
        }
        if (response.detail_options.yelp_places == 'on') {
            jQuery('#wp-rem-property-yelp-places-holder').show();
        } else {
            jQuery('#wp-rem-property-yelp-places-holder').hide();
        }
        if (response.detail_options.floor_plans == 'on') {
            jQuery('#wp-rem-property-floor-plans-holder').show();
        } else {
            jQuery('#wp-rem-property-floor-plans-holder').hide();
        }
        if (response.detail_options.appartments == 'on') {
            jQuery('#wp-rem-property-appartments-holder').show();
        } else {
            jQuery('#wp-rem-property-appartments-holder').hide();
        }
        if (response.detail_options.features == 'on') {
            jQuery('#wp-rem-property-features-holder').show();
        } else {
            jQuery('#wp-rem-property-features-holder').hide();
        }
        if (response.detail_options.video == 'on') {
            jQuery('#wp-rem-property-video-holder').show();
        } else {
            jQuery('#wp-rem-property-video-holder').hide();
        }
        if (response.detail_options.virtual_tour == 'on') {
            jQuery('#wp-rem-property-virtual-tour-holder').show();
        } else {
            jQuery('#wp-rem-property-virtual-tour-holder').hide();
        }
        if (response.detail_options.faqs == 'on') {
            jQuery('#wp-rem-property-faqs-holder').show();
        } else {
            jQuery('#wp-rem-property-faqs-holder').hide();
        }
    ...
    if (window.File && window.FileList && window.FileReader) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (!file.type.match('image')) {
                jQuery.growl.error({
                    message: wp_rem_property_strings.upload_images_only
                });
                image_file = false;
            }
...
function wp_rem_handle_file_select(event, counter) {
    ...
    if (window.File && window.FileList && window.FileReader) {
        ...
        if (images_num_allowed == 0) {
            var images_num_allowed = $('.dev-property-pakcge-step').data('picnum');
        ...
        if (images_added_length <= images_num_allowed) {
            ...
            jQuery.growl.error({
                message: wp_rem_property_strings.more_than_f + " " + images_num_allowed + " " + wp_rem_property_strings.more_than_image_change
            });
        }
    } else {
        jQuery.growl.error({
...
function wp_rem_handle_attach_file_select(event, counter) {
    ...
    if (window.File && window.FileList && window.FileReader) {
        ...
        if (docs_num_allowed == 0) {
            var docs_num_allowed = $('.dev-property-pakcge-step').data('docnum');
        ...
        if (docs_added_length <= docs_num_allowed) {
            ...
            for (var i = 0; i < files.length; i++) {
                ...
                if (jQuery.inArray(file_ext, allowd_extnx) == -1) {
                        ...
                        <div class="modal-dialog" role="document">\
                            <div class="modal-content">\
                                <div class="modal-header">\
                                    ...
                                    <h4 class="modal-title" id="faqModalLabel">' + wp_rem_property_strings.wp_rem_edit_details + '</h4>\
                                </div>\
                                <div class="modal-body">\
                                    <div class="row">\
                                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">\
                                            <div class="field-holder">\
                                                <input type="text" placeholder="Title *" class="form-control" id="wp_rem_property_attachment_title" name="wp_rem_property_attachment_title[]"></div>\
                                        </div>\
                                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">\
                                            <div class="field-holder faq-request-holder input-button-loader">\
                                                <input type="button" class="bgcolor wp_rem_add_attachment_data" name="add_attachment_data" data-id="' + timestamp + '" value="' + wp_rem_property_strings.wp_rem_edit_details_update + '"></div>\
                                        </div>\
                                    </div>\
                                </div>\
                            </div>\
                    ...
                    ' + popup_html + '</li>' + document.getElementById("wp-rem-dev-docs-attach-sec-" + counter).innerHTML;
            ...
            jQuery.growl.error({
                message: wp_rem_property_strings.more_than_f + " " + docs_num_allowed + " " + wp_rem_property_strings.more_than_doc_change
            });
        }
    } else {
        jQuery.growl.error({
                    ...
                    <div class="modal-content">\
                        ...
                        <div class="modal-body">\
                            <div class="row">\
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">\
                                    <div class="field-holder">\
                                        <input type="text" placeholder="Title *" class="form-control" id="wp_rem_property_floor_plan_title" name="wp_rem_property_floor_plan_title[]"></div>\
                                </div>\
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">\
                                    <div class="field-holder">\
                                        <textarea placeholder="Description *" class="form-control" rows="5" cols="30" id="wp_rem_property_floor_plan_desc" name="wp_rem_property_floor_plan_desc[]"></textarea></div>\
                                </div>\
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">\
                                    <div class="field-holder faq-request-holder input-button-loader">\
                                        <input type="button" class="bgcolor wp_rem_add_floor_plan_data" name="add_floor_plan_data" data-id="' + timestamp + '" value="' + wp_rem_property_strings.wp_rem_edit_details_update + '"></div>\
                                </div>\
                            </div>\
                        </div>\
                    </div>\
                        ...
                        ' + popup_html + '</li>' + document.getElementById("wp-rem-dev-floor-attach-sec-" + counter).innerHTML;
                    }
...
$(document).on('click', '.wp_rem_add_floor_plan_data', function () {
    var data_id = jQuery(this).data('id');
    var title = jQuery(this).closest('.modal-body').find('#wp_rem_property_floor_plan_title').val();
    jQuery(".add-floor-data-link-"+data_id).html(title);
    $('#add-floor-image-data-'+data_id).modal('hide');
});

$(document).on('click', '.wp_rem_add_attachment_data', function () {
    var data_id = jQuery(this).data('id');
    var title = jQuery(this).closest('.modal-body').find('#wp_rem_property_attachment_title').val();
...
jQuery(document).on("click", "#add-faq #wp_rem_add_faq_to_list", function () {
    "use strict";
    
    var thisObj = jQuery(this);
    var this_loader_Obj = jQuery(".faq-request-holder");
    wp_rem_show_loader(".faq-request-holder", "", "button_loader", this_loader_Obj);

    thisObj.prop('disabled', true);
    var faq_title = jQuery('#add-faq #wp_rem_faq_title').val();
    var faq_desc = jQuery('#add-faq #wp_rem_faq_desc').val();
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            if (response.type == 'success') {
                jQuery('.property-faq-list').append(response.html);
                jQuery('#add-faq .close-faq').click();
                jQuery('#add-faq #wp_rem_faq_title').val('');
                jQuery('#add-faq #wp_rem_faq_desc').val('');
            }
        }
    });
    return false;
...
jQuery(document).on("click", ".property-faq-list .edit-faq", function () {
    "use strict";
    var thisObj = jQuery(this);
    var faq_title = thisObj.closest('li').find('#wp_rem_faq_title').val();
    var faq_desc = thisObj.closest('li').find('#wp_rem_faq_desc').val();
    var faq_counter = thisObj.closest('li').find('#wp_rem_faq_counter').val();
    jQuery('#edit-faq').modal('show');
    jQuery('#edit-faq').find('#wp_rem_faq_title').val(faq_title);
    jQuery('#edit-faq').find('#wp_rem_faq_desc').val(faq_desc);
    jQuery('#edit-faq').find('#wp_rem_faq_counter').val(faq_counter);
});

jQuery(document).on("click", "#wp_rem_edit_faq_to_list", function () {
    "use strict";
    
    var thisObj = jQuery(this);
    var this_loader_Obj = jQuery(".edit-faq-request-holder");
    wp_rem_show_loader(".edit-faq-request-holder", "", "button_loader", this_loader_Obj);
    
    thisObj.prop('disabled', true);
    var faq_title = jQuery('#edit-faq #wp_rem_faq_title').val();
    var faq_desc = jQuery('#edit-faq #wp_rem_faq_desc').val();
    var faq_counter = jQuery('#edit-faq #wp_rem_faq_counter').val();
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            wp_rem_show_response(response, "", this_loader_Obj);
            if (response.type == 'success') {
                jQuery('.property-faq-list .faq-'+ faq_counter).html();
                jQuery('.property-faq-list .faq-'+ faq_counter).html(response.html);
                jQuery('#edit-faq').modal('hide');
                jQuery('#edit-faq #wp_rem_faq_title').val('');
                jQuery('#edit-faq #wp_rem_faq_desc').val('');
            }
        }
    });
    return false;
...
jQuery(document).on("click", ".property-faq-list .remove-faq a", function() {
    "use strict";
    var thisObj = jQuery(this);
...
jQuery(".apartment_repeater_btn").live('click', function () {
    ...
    jQuery(this).append(' <span class="loader-spinner"><i class="icon-spinner8"></i></span>');
    jQuery.ajax({
        type: "POST",
        url: wp_rem_globals.ajax_url,
        data: dataString,
